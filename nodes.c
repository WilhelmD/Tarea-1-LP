	...
	if(*ptrarbol==NULL){
		...
		*ptrCabecera=ptrAuxLista;
		...
		if(ptrAuxArbol->children==NULL && contador<largo_array){
			...
			for (; (**ptrIterador)->data->word != palabras[contador] && *(ptrIterador)->siguiente != NULL; *ptrIterador = (*ptrIterador)->siguiente);{
				if((*ptrIterador)->data->word == palabras[contador]){
